...
export class LogoSanitizerService {
   ...
   */
  sanitize(file: { buffer: Buffer; mimetype: string; originalname?: string }): SanitizedLogo {
   ...
   */
  private detectByMagicBytes(buf: Buffer): 'png' | 'jpg' | 'webp' | 'svg' | null {
  ...
  private sanitizeSvg(raw: string): string {
    ...
    const blacklist = [
      '<script', '<foreignobject', '<iframe', 'javascript:', 'vbscript:',
