...
} from "../../lib/contract-preview-cancellation";
...
export default function CreateContractPage() {
  ...
  const [previewCancellationError, setPreviewCancellationError] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    const loadPrefillData = async () => {
      ...
      try {
        ...
        setValue("buyer_rg", data.buyer.rg ? applyRgMask(data.buyer.rg) : "");
  ...
  const buildContractData = (
    ...
    platform_checking_account: formData.platform_checking_account || undefined,
    // Office split
  ...
  const onPreview = async (formData: ContractFormData) => {
    ...
    try {
      ...
      if (isEmailConflict) {
        ...
        setSubmitStatus({
          type: "error",
          message: backendMessage || "Erro ao criar preview. Tente novamente.",
        });
      }
    } finally {
  ...
  const handleConfirmSend = useCallback(async () => {
    if (!previewData || !previewFormData || previewActionInFlightRef.current)
      return;
  ...
  const cancelAndDiscardPreview = useCallback(async (
    mode: "cancel" | "expire",
  ) => {
    ...
    try {
      ...
      setPreviewCancellationError(
        mode === "expire"
          ...
          : "Não foi possível cancelar o preview. Ele foi mantido aberto; tente novamente.",
      );
    } finally {
      previewActionInFlightRef.current = false;
      setIsCancellingPreview(false);
    }
  }, [previewData, processId]);

  // Handler para cancelar preview
  const handleCancelPreview = useCallback(async () => {
    await cancelAndDiscardPreview("cancel");
  }, [cancelAndDiscardPreview]);

  // Handler para preview expirado
  const handlePreviewExpired = useCallback(async () => {
    await cancelAndDiscardPreview("expire");
  }, [cancelAndDiscardPreview]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        ...
        <span className="ml-2 text-muted">Carregando dados do contrato...</span>
      </div>
    );
  }
    ...
    <div className={`${isMobile ? "px-4 py-6" : "px-8 py-8"}`}>
          ...
          <form onSubmit={handleSubmit(onPreview)} className="space-y-8">
            ...
            <section className="bg-surface rounded-lg border border-border p-6">
              ...
              <div className="relative" ref={templateDropdownRef}>
                ...
                >
                  <span
                    className={
                      selectedTemplate ? "font-medium text-ink" : "text-subtle"
                    }
                  >
                    {selectedTemplate
            ...
            <div>
              ...
              <section className="bg-surface rounded-lg border border-border p-6 mb-6">
                <div className="flex items-center justify-between border-b pb-2 mb-4">
                  <h3 className="text-base font-semibold text-ink">Vendedor</h3>
            ...
            <section className="bg-surface rounded-lg border border-border p-6">
              ...
              <p className="text-sm text-muted mb-4">
                Adicione até duas testemunhas para o contrato. Ambos os campos
                são opcionais.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Testemunha 1 */}
      ...
      {showPreviewModal && previewData && (
        <DocuSignPreviewModal
          ...
          isCancelling={isCancellingPreview}
          cancellationError={previewCancellationError}
        />
      )}
    </div>
  );
